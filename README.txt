...
Section 1: Operation Instructions
	...
	Part 1.3: Dump and Combine
		...
		1.3.3 Open up each of the data.win files (with the exception of output\xDeltaCombiner\1\data.win), then hit "Scripts" on the top left. Run "ExportAllCode.csx" and "ExportAllTilesets.csx" (only assets types that were tested)
		...
		1.3.8 Open up output\xDeltaCombiner\1\data.win in UTMT, and run the scripts "ImportGML.csx" and "ImportAllTileSets.csx", choose output\xDeltaCombiner\1\Objects as the selected folder
		1.3.9 Hit File -> Save and then hit enter. It will ask if you want to replace it, hit "yes"
		1.3.10 Your complete data.win is located at output\xDeltaCombiner\1\data.win
		1.3.11 Once you hit enter again, the program will delete output\xDeltaCombiner and close. 
