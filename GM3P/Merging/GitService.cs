        ...
        private string AutoResolveConflicts(string mergedText, string relativePath, GM3PConfig config)
        {
            ...
            string ReplaceOne(string input)
            {
                return rx.Replace(input, m =>
                {
                    ...
                    Console.WriteLine(both);//Debug: show both sides of the conflict
                    ...
                    {
                        File.WriteAllText(Path.Combine(config.OutputPath,"xDeltaCombiner","merge.txt"), local + "\n" + remote); //Debug: show both sides of the conflict
