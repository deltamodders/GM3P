...
// UTMT v0.8.x port by CST1229

// my god this is the worst way to parse a json document i've ever seen
// *why the heck does this use positioning?!?!? that defeats the whole point of json...*
// (whatever i just want to get a script out that works since utmtce is dead now)
// -CST
...
string importFolder = @Convert.ToString(Directory.GetParent(Convert.ToString(Directory.GetParent(Convert.ToString(Assembly.GetEntryAssembly().Location))))) + "/output/xDeltaCombiner/"+chapterNo+"/"+modNo+"/Objects/Rooms";
